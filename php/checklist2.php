...
if ($phone != "" ){
    ...
    $sql="select * from alarmlist AS c left outer join user AS u on u.const_site=c.const_site where u.phone='$phone'";
   ...
   if ($stmt->rowCount() == 0)
	...
	if (!$android) {
        ...
        }else
        {
            ...
            $json = json_encode(array("response"=>$data), JSON_PRETTY_PRINT+JSON_UNESCAPED_UNICODE);
            echo $json;
            $sql="delete a from alarmlist a left join user on user.const_site=a.const_site where user.phone='$phone'";
            $stmt = $con->prepare($sql);
            $stmt->execute();
                   }
    }
